...
const MensajesMotivacion: React.FC = () => {
  const [motivationMessages, setMotivationMessages] = useState<Template[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      const messages = await getTemplates();
      setMotivationMessages(messages.filter(t => t.name.includes('Motivación')));
  ...
  const handleClientTypeChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const messages = await getTemplates(typeId);
    setMotivationMessages(messages.filter(t => t.name.includes('Motivación')));
  };

  return (
    <div className="p-4 bg-white shadow rounded-lg">
      <h2 className="text-2xl font-bold text-gray-800 mb-4">Mensajes de Motivación</h2>
      <p className="text-gray-600 mb-6">Mensajes de motivación para diferentes momentos del journey del cliente.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {motivationMessages.map((message) => (
          <div key={message.id} className="border border-gray-200 rounded-lg p-4 shadow-sm">
            <h3 className="text-xl font-semibold text-gray-700 mb-2">{message.name}</h3>
            <p className="text-sm text-gray-500 mb-2">Tipo: {clientTypes.find(ct => ct.id === message.clientTypeId)?.name}</p>
            <p className="text-gray-700 text-sm line-clamp-3">{message.content}</p>
            <div className="mt-3 flex flex-wrap gap-2">
              {message.dynamicVariables.map(varId => {
                const variable = dynamicVariables.find(v => v.id === varId);
                return variable ? (
                  <span key={varId} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                    {variable.name}
                  </span>
                ) : null;
              })}
      ...
      <button className="mt-6 w-full inline-flex items-center justify-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
        Crear Nuevo Mensaje de Motivación
      </button>
    </div>
  );
};

export default MensajesMotivacion;
