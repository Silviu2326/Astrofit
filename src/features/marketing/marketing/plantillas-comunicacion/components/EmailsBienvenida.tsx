...
const EmailsBienvenida: React.FC = () => {
  const [welcomeEmails, setWelcomeEmails] = useState<Template[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      const emails = await getTemplates();
      setWelcomeEmails(emails.filter(t => t.name.includes('Bienvenida')));
  ...
  const handleClientTypeChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const emails = await getTemplates(typeId);
    setWelcomeEmails(emails.filter(t => t.name.includes('Bienvenida')));
  };

  return (
    <div className="p-4 bg-white shadow rounded-lg">
      <h2 className="text-2xl font-bold text-gray-800 mb-4">Emails de Bienvenida</h2>
      <p className="text-gray-600 mb-6">Emails de bienvenida personalizables para nuevos clientes.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {welcomeEmails.map((email) => (
          <div key={email.id} className="border border-gray-200 rounded-lg p-4 shadow-sm">
            <h3 className="text-xl font-semibold text-gray-700 mb-2">{email.name}</h3>
            <p className="text-sm text-gray-500 mb-2">Tipo: {clientTypes.find(ct => ct.id === email.clientTypeId)?.name}</p>
            <p className="text-gray-600 text-sm mb-3">Asunto: {email.subject}</p>
            <p className="text-gray-700 text-sm line-clamp-3">{email.content}</p>
            <div className="mt-3 flex flex-wrap gap-2">
              {email.dynamicVariables.map(varId => {
                const variable = dynamicVariables.find(v => v.id === varId);
                return variable ? (
                  <span key={varId} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                    {variable.name}
                  </span>
                ) : null;
              })}
      ...
      <button className="mt-6 w-full inline-flex items-center justify-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
        Crear Nuevo Email de Bienvenida
      </button>
    </div>
  );
};

export default EmailsBienvenida;
