...
import { Video, Play, Tag, Sparkles, AlertCircle, Loader2 } from 'lucide-react';
...
import { getVideos } from '../bibliotecaContenidosApi';
import { VideoResponse } from '../types/index';
...
const ContenidosVideoPage: React.FC = () => {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50/30 to-purple-50/30 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg text-gray-600">Cargando videos...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <ApiErrorPage
        error={`Error al cargar videos: ${error}`}
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50/30 to-purple-50/30 pb-12">
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <h1 className="text-4xl md:text-6xl font-bold text-white tracking-tight">
              Biblioteca <span className="bg-clip-text text-transparent bg-gradient-to-r from-yellow-200 to-yellow-400">Videos</span>
            </h1>
          </div>

          {/* Descripción */}
          <p className="text-xl md:text-2xl text-blue-100 max-w-3xl leading-relaxed">
            Gestiona tu videoteca con <span className="font-bold text-white px-2 py-1 bg-white/20 rounded-lg backdrop-blur-sm">organización inteligente</span> y etiquetas personalizadas
          </p>
          ...
          <div className="mt-8 flex flex-wrap gap-4">
            <div className="flex items-center gap-2 bg-white/10 backdrop-blur-md rounded-full px-4 py-2 border border-white/20">
              ...
              <span className="text-sm font-semibold text-white">Reproducción HD</span>
            </div>
            <div className="flex items-center gap-2 bg-white/10 backdrop-blur-md rounded-full px-4 py-2 border border-white/20">
              <Tag className="w-5 h-5 text-blue-300" />
              <span className="text-sm font-semibold text-white">Etiquetas Inteligentes</span>
            </div>
            <div className="flex items-center gap-2 bg-white/10 backdrop-blur-md rounded-full px-4 py-2 border border-white/20">
              <Sparkles className="w-5 h-5 text-purple-300" />
              <span className="text-sm font-semibold text-white">Videoteca Privada</span>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <div className="space-y-8">
        ...
        >
          ...
          <div className="relative z-10">
            <EtiquetasVideo onSelectTag={setSelectedTag} />
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          >
            <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-xl p-6 border border-white/50 relative overflow-hidden h-full">
              ...
              <div className="relative z-10">
                <GaleriaVideos selectedTag={selectedTag} />
              </div>
            </div>
          </motion.div>
