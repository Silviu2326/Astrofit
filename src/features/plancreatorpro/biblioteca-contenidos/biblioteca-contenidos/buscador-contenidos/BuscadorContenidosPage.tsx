...
import { Search, Filter, FileText, AlertCircle, Loader2 } from 'lucide-react';
...
import { searchContent } from '../bibliotecaContenidosApi';
...
const BuscadorContenidosPage: React.FC = () => {
  ...
  // Cargar todos los contenidos al inicio usando una búsqueda válida
  useEffect(() => {
    let isMounted = true;
    ...
    const loadAllContent = async () => {
      ...
      setIsSearching(true);
      try {
        // Usar una búsqueda que devuelva todos los resultados
        const results = await searchContent('a', {}); // Búsqueda mínima válida para obtener todo
        if (isMounted) {
  ...
  const handleApplyFilters = async (filters: any) => {
    ...
    setHasSearched(true);
    try {
      const results = await searchContent(searchQuery, filters);
      setSearchResults(results.resultados || []);
    } catch (error) {
  ...
  const handleClearFilters = async () => {
    ...
    setHasSearched(true);
    try {
      // Usar búsqueda mínima válida para obtener todos los resultados
  ...
  if (error) {
    return (
      <ApiErrorPage
        error={`Error en el buscador de contenidos: ${error}`}
        onRetry={() => {
          setError(null);
