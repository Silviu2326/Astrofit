...
import { Users, UserCheck, DollarSign, CreditCard, ArrowUpRight, ArrowDownRight } from 'lucide-react';
import { mockAffiliates } from '../data/mockData';

export const QuickStats: React.FC = () => {
  const totalAffiliates = mockAffiliates.length;
  const activeAffiliates = mockAffiliates.filter(a => a.status === 'Activo').length;
  const monthSales = mockAffiliates.reduce((sum, a) => sum + a.salesGenerated, 0);
  const pendingCommissions = mockAffiliates.reduce((sum, a) => sum + (a.pendingCommission || 0), 0);
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <div className="flex items-center gap-2">
              <div className={`p-1 ${stat.isPositive ? 'bg-green-50' : 'bg-red-50'} rounded-lg`}>
                {stat.isPositive ? (
