...
echo.
echo Iniciando envio de prompts a Gemini...
echo ========================================

:: Use PowerShell to split prompts and execute Gemini with 5-minute timeout and 90s delays
powershell -Command "& { $prompts = (Get-Content 'promptsgemini.txt' -Raw) -split '(?m)^---\s*$' | Where-Object { $_.Trim().Length -gt 0 }; Write-Host \"Total de prompts encontrados: $($prompts.Count)\" -ForegroundColor Cyan; Write-Host \"⏱️ Timeout configurado: 5 minutos por prompt\" -ForegroundColor Yellow; Write-Host \"⏰ Delay entre prompts: 90 segundos\" -ForegroundColor Yellow; for ($i = 0; $i -lt $prompts.Count; $i++) { $promptNum = $i + 1; Write-Host \"`nTurno $promptNum/$($prompts.Count)\" -ForegroundColor Yellow; Write-Host \"Inicio: $(Get-Date -Format 'HH:mm:ss')\" -ForegroundColor Gray; $logFile = \"geminilogs\%timestamp%-turn$promptNum.txt\"; Write-Host \"Enviando prompt a Gemini...\" -ForegroundColor Cyan; $job = Start-Job -ScriptBlock { param($prompt) $prompt | & gemini -m gemini-1.5-flash --yolo } -ArgumentList $prompts[$i]; $completed = Wait-Job $job -Timeout 300; if ($completed) { $response = Receive-Job $job; $response | Out-File -FilePath $logFile -Encoding UTF8; Write-Host $response; Write-Host \"Respuesta guardada en: $logFile\" -ForegroundColor Gray; Write-Host \"Turno $promptNum completado\" -ForegroundColor Green; } else { Stop-Job $job; Write-Host \"⚠️ Timeout alcanzado (5 min) - pasando al siguiente prompt\" -ForegroundColor Red; \"TIMEOUT: Prompt excedió 5 minutos de ejecución`nPrompt original:`n$($prompts[$i])\" | Out-File -FilePath $logFile -Encoding UTF8; } Remove-Job $job -Force -ErrorAction SilentlyContinue; if ($i -lt ($prompts.Count - 1)) { Write-Host \"⏸️ Esperando 90 segundos antes del siguiente prompt...\" -ForegroundColor Magenta; Start-Sleep -Seconds 90; } Write-Host \"------------------------------------------------------------\" -ForegroundColor DarkGray; } }"
